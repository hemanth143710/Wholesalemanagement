...
from models.index import students,orders,stock,buyers,customer
app=FastAPI()

# best way to make api
@app.get('/api/stock')
async def getindex():
    data=con.execute(stock.select()).fetchall()
    return {
        "success": True,
        "data":data
    }
...
@app.get('/api/orders')
async def getorders():
    data=con.execute(orders.select()).fetchall()
    return {
        "success": True,
        "data":data
    }
@app.get('/api/buyers')
async def getbuyers():
    data=con.execute(buyers.select()).fetchall()
    return {
        "success": True,
        "data":data
    }

# @app.get('/api/costomers')
# async def getcostomers():
#     data=con.execute(customer.select()).fetchall()
#     return {
#         "success": True,
#         "data":data
#     }
